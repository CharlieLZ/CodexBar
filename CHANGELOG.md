...
### Fixed
- Codex: report the app-server's own failure instead of a fabricated RPC timeout, so an expired ChatGPT token reads "Needs re-auth" rather than "Unavailable" and no longer re-enters the connectivity retry schedule.
